...
function/**/scheduleCooldown(v){let/**/r=Math.max(0,Math.ceil(Number(v)||0));chargeState.remaining=r;chargeState.active=r>0;if(chargeState.retryTimer)clearTimeout(chargeState.retryTimer);if(!r){stopChargeMode();return;}renderCharge('Cooldown:'+r+'s');chargeState.retryTimer=setTimeout(()=>{stopChargeMode();syncCooldown();},r*1000);}
async/**/function/**/syncCooldown(){let/**/k=savedKey()||apiKeyValue();if(!k){stopChargeMode();return;}try{let/**/r=await/**/get(k,{action:'cooldown',player:currentPlayer()});let/**/v=r.cooldown_seconds||r.remaining_seconds||r.cooldown||0;if(v)scheduleCooldown(v);else/**/stopChargeMode();}catch(e){if(e.status!==429)stopChargeMode();}}
...
async function logMeal(event) { event.preventDefault(); if (chargeState.active) return; startCharge(850); setBusy(true); try { const key = savedKey() || apiKeyValue(); if (!key) throw new Error('Add the API key in Settings first.'); storePlayer(); await new Promise(resolve => setTimeout(resolve, 850)); const result = await get(key, { action: 'log', ...formValues(), player: playerValue() }); const effects = await loadAll(); renderBuff(effects); setConnected(true); const level = result.stats?.level; setStatus(level === undefined ? 'Logged!' : 'Logged! +' + result.xp_gained + ' XP level ' + level + '.', true); flashSuccess(); } catch (error) { if (error && error.status === 429) { const remaining = error.remaining_seconds || error.cooldown_seconds || 0; scheduleCooldown(remaining); setStatus('Cooldown: ' + remaining + 's. Try again after it clears.', true); } else { setConnected(false); setStatus(error.message); } } finally { setBusy(false); stopChargeMode(); } }
